 ...
 * @return Un tableau de deux joueurs.
 */
	public static HumanPlayer getName1()
	{
		Scanner		sc;
		HumanPlayer player1;
 ...
 */
    public static Move getMove(String name, int[] tab)
	{
		Scanner 		sc;
		boolean			btest;
		Move 			currentMove;
		...
		do 
		{
			try 
			{
			    while(val == false)
			    {
				System.out.println("Entrer une ligne et un nombre d allumette convenant au tableau.");
				...
				if(line > 0 && line <= tab.length && tab[line - 1] >= match && match <= 3 && match > 0 )
				    val = true;
			    }
			}
    ...
    public static Move getMove(String name, Move currentMove)
    {
	System.out.println("C'est à " + name + " de jouer :");
	System.out.println(name + " à choisie , line : " + (currentMove.getLine() + 1) + ", nombre d allumette : " +currentMove.getMatchNb());
